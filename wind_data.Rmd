...
agg_u_vecs <- raster::aggregate(wind_u_brick, 2, mean) %>% 
...
agg_v_vecs <- raster::aggregate(wind_v_brick, 2, mean) %>% 
  mask(merged_shapes, inverse = T) %>% 
  rasterToPoints() %>% 
  as.data.frame() %>% 
  pivot_longer(!c(x,y), names_to = "date", values_to = "v") %>% 
...
for(i in seq_along(wind.list)){
  ...
  new.wind.list[[i]] <- wind.list[[i]] +
    geom_text(data = vec[vec$date==unique(vec$date)[i],], aes(angle=-angle+90), label="→", check_overlap = T, size = 2)
}
