...
wind_48 <- wind_ras %>% 
  filter(date == unique(date)[[48]]) %>% 
  rasterFromXYZ()%>% 
  dropLayer(2)

wind_49 <- wind_ras %>% 
  filter(date == unique(date)[[49]]) %>% 
  rasterFromXYZ()%>% 
  dropLayer(2)

wind_50 <- wind_ras %>% 
  filter(date == unique(date)[[50]]) %>% 
  rasterFromXYZ()%>% 
  dropLayer(2)

wind_51 <- wind_ras %>% 
  filter(date == unique(date)[[51]]) %>% 
  rasterFromXYZ()%>% 
  dropLayer(2)

wind_52 <- wind_ras %>% 
  filter(date == unique(date)[[53]]) %>% 
  rasterFromXYZ()%>% 
  dropLayer(2)
...
ras_brick <- brick(wind_1, wind_2, wind_3, wind_4, wind_5, wind_6, wind_7,
               ...
               wind_46, wind_47, wind_48, wind_49, wind_50, wind_51, wind_52, wind_53)

names(ras_brick) <-  date
...
wind_pal <- colorRampPalette(c( "#5f6fb3", "#498f9e", "#4a8b6c","goldenrod2", "#93525a", "#944988", "firebrick4" ))
