...
cords_1 <- test %>% ungroup %>% 
  dplyr::select(x,y) %>% 
  SpatialPoints()

idw <- gstat(id = "wind", formula = wind ~ 1, locations = ~x+y, data = cords)



a <- rasterize(cords_1, new_ras_wind, field = test$wind) 
...
ggplot(test) +
  geom_tile(aes(x=x, y=y, fill = wind, interpolate = T))

```

```{r}
crs = 4326
sf_data <- st_as_sf(test, coords = c('x', 'y'), crs = 4326) %>% 
  cbind(st_coordinates(.))


#vario gram and model selection 
jan_np_vario <- variogram(wind~1, as(sf_data, "Spatial"))

plot(jan_np_vario)

v_mod_full <- fit.variogram(jan_np_vario, vgm(psill = 150, "Exp", range = 200, nugget = 0.1))
...
new_ras_wind <- raster(xmn = -121.40,
                      xmx = -117.06,
                      ymn = 32.36,
                      ymx = 34.75,
                      res = wind_res)
...
for(i in unique(wind_ras$date)) {
  ...
  rs <- addLayer(r, rasterFromXYZ(wind_ras[wind_ras$date == unique(wind_ras$date)[i],]))
}
